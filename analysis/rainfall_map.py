    ...
    for tok in tokens:
        ...
        if p.is_dir():
            paths += sorted(q for q in p.iterdir()
                            if q.suffix.lower() in (".csv", ".txt", ".tsv", ".parquet", ".json"))
...
function render(rebuildSurface){
  ...
  const nOdd = vals.filter(odd).length;
  document.getElementById('qa').textContent = nOdd
    ? nOdd + ' gauge' + (nOdd>1?'s':'') + ' far from the network median — check for outage or fault'
    : '';

  const rows = [...t].sort((a,b)=> b.total-a.total);
  document.getElementById('ranked').innerHTML = rows.map(d => {
    const frac = hi>0 ? d.total/hi : 0;
    const warn = odd(d.total)
      ? ` <span title="${(d.total/med).toFixed(1)}x the network median for this window" style="color:#c2532b">&#9888;</span>` : '';
    return `<tr class="${selected===d.id?'sel':''}" data-id="${d.id}">
      <td>${d.name}${warn}<br><span class="sub">${d.borough}</span></td>
