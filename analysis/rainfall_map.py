...
<div class="controls">
  <div class="brushwrap">
    <div class="brushlabel">
      <span>Citywide median depth &mdash; drag to select</span>
...
function totals(){
  ...
  return DATA.gauges.map(g => {
    ...
    for (let i = i0; i <= i1; i++){
      ...
      if (f === '1') fault++; else if (f === '2') out++;
      if (f === '0' || raw){ s += g.series[i]; ok++; }
    }
...
function render(rebuildSurface){
  ...
  document.getElementById('r-window').textContent =
    match ? labelFor(match) : DATA.periods[i0] + ' → ' + DATA.periods[i1];
...
function drawHyeto(){
  ...
  EV.forEach(e => {
    if (!e.name) return;
    const x = xOf(e.i0) + (xOf(e.i1+1) - xOf(e.i0))/2;
    hx.fillStyle = '#8a999f';
    hx.fillRect(x - 0.5, 0, 1, 5);
  });
...
def find_events(binned: pd.DataFrame, flags: pd.DataFrame, periods: list,
    ...
    events = []
    for a, b in runs:
        depth = float(med.iloc[a:b + 1].sum())
...
def main():
    ...
    qa.add_argument("--corroboration", type=float, default=0.15,
                    help="share of the reading a peer gauge must also catch (default: 0.15)")
